...
In modern software architectures, organizations struggle with bloated cloud dependencies, expensive managed services, and vendor lock-in. **PacketPulse-Sniffer** provides a self-hosted, lightweight, sub-millisecond solution crafted from first principles with zero external runtime dependencies.
...
```mermaid
flowchart TD
    Client["🌐 Client Applications / Microservices"] -->|HTTP REST / JSON| Gateway["⚡ PacketPulse-Sniffer Entrypoint (Port 6020)"]
...
- **`GET /api/health`**: Health status and uptime
  ```bash
  curl -X GET http://localhost:6020/api/health
  ```
- **`GET /api/stats`**: Operational metrics and engine telemetry
  ```bash
  curl -X GET http://localhost:6020/api/stats
  ```
- **`POST /api/process`**: Execute computational logic against engine
  ```bash
  curl -X POST http://localhost:6020/api/process \
...
| :--- | :--- | :--- |
| `PORT` | `6020` | HTTP listening port for REST API and Web Dashboard |
